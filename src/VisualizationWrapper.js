...
const VisualizationWrapper = ({
  ...
  useEffect(() => {
    const mappingPromise = json(`https://sammenlign.utdanning.no/rest/${direction}?uno_id=${unoId}&dataset=total`)
    const allDataPromise = json(`https://sammenlign.utdanning.no/rest/main?uno_id=${unoId}`)
    Promise
